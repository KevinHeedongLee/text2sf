...
#if defined( __gnu_linux__ )
   /* PCAP timestamps should be in GMT. */
   timezone.tz_minuteswest = 0;
   timezone.tz_dsttime = 0;

   gettimeofday( &timestamp, &timezone );
   ptr->record_header.sec = timestamp.tv_sec;
   ptr->record_header.usec = timestamp.tv_usec;
#elif defined( _WIN32 )
   /*
   ** Not really needed, but Windows does not seem to 'refresh' the
   ** returned FILETIME value otherwise, resulting in all entries in
   ** the PCAP having more or less the same time stamp...
   */
   Sleep( 0 );
